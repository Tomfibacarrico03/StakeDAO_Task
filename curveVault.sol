...
contract CurveStrategy is BaseStrategy {
    ...
    address public sdtDistributor;
    address public constant CRV_FEE_D =
        0xA464e6DCda8AC41e03616F95f4BC98a13b8922Dc;
    address public constant CRV3 = 0x6c3F90f043a72FA612cbac8115EE7e52BDe6E490;
    address public constant CRV_MINTER =
    ...
    function sendToAccumulator(address _token, uint256 _amount)
        external
        onlyGovernance
    {
    ...
    function claim(address _token) external override {
        ...
        if (
            lGaugeType[gauge] == 0 &&
            ILiquidityGauge(gauge).reward_tokens(0) != address(0)
        ) {
            (success, ) = locker.execute(
                gauge,
                0,
            ...
            for (uint8 i = 0; i < 8; i++) {
                ...
                if (success) {
                    rewardsBalance = IERC20(rewardToken).balanceOf(
                        address(this)
                    );
                } else {
                    rewardsBalance = IERC20(rewardToken).balanceOf(
    ...
    function sendFee(
        ...
        uint256 accumulatorPart = (_rewardsBalance * accumulatorFee[_gauge]) /
            BASE_FEE;
        uint256 veSDTPart = (_rewardsBalance * veSDTFee[_gauge]) / BASE_FEE;
        uint256 claimerPart = (_rewardsBalance * claimerRewardFee[_gauge]) /
            BASE_FEE;
        // send
    ...
    function setLGtype(address _gauge, uint256 _gaugeType)
        external
        onlyGovernanceOrFactory
    {
...
contract FixedAPY is ERC20Upgradeable {
    using SafeERC20Upgradeable for ERC20Upgradeable;
    using AddressUpgradeable for address;

    ERC20Upgradeable public token;
    CurveVault public curveVault;
    uint256 public apyR; //APY Retail
	...
	function withdraw (uint256 _shares) public {
		...
		require(executionTime >= depositTime + 90 days);

		//Passed Time since the deposit in months
		uint256 monthsDifference = (executionTime - depositTime)/ 60 / 60 / 24/ 30; 
		uint256 userTotalShares = ILiquidityGaugeStrat(liquidityGauge).balanceOf(msg.sender);
		
		require(_shares > userTotalShares);
		uint256 amountToWithdraw = _shares + (_shares/((uint256(19)/uint256(12))*monthsDifference));
	...
	function setCurveVault(CurveVault _curveVault) public {
		require(msg.sender == governance, "!governance");
		require(address(_curveVault) != address(0), "zero address");
		require(address(curveVault) == address(0), "Vault already set");
